...
public class FalsaPosicion {
    ...
    public static void falsaPosicion(double a, double b, double tolerancia, int maxIteraciones, Function<Double, Double> funcion) {
        ...
        System.out.printf("%-10s %-12s %-12s %-12s %-12s %-12s%n",
                "Iteración", "Valor a", "Valor b", "Xi", "f(Xi)", "Error Relativo (%)");
        ...
        while (iteracion <= maxIteraciones) {
            ...
            System.out.printf("%-10d %-12.6f %-12.6f %-12.6f %-12.6f ", iteracion, a, b, c, fC);

            // Calcular el error relativo
            double errorRelativo = c-anteriorC;

            // Mostrar el error relativo
            System.out.printf("%-12.6f%n", errorRelativo);
    ...
    public static void main(String[] args) {
        // Definir la función
        Function<Double, Double> funcion = x -> Math.pow(x,3)+x-6;

        // Parámetros para el método
        double a = 4; // Límite inferior
        double b = 4.5; // Límite superior
